...
namespace TinyGiantStudio.BetterInspector.BetterMesh
{
    ...
    public class SkinnedMeshRendererEditor : Editor
    {
        ...
        [SerializeField] VisualTreeAsset meshPreviewTemplate;

        const string MeshPreviewTemplateFileLocation =
            "Assets/Plugins/Tiny Giant Studio/Better Inspector/Better Mesh Filter/Scripts/Editor/Templates/MeshPreview.uxml";
        const string MeshPreviewTemplateGuid = "7358ad733b4a05a4c88dbecb820153e8";
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            // Debug.Log(AssetDatabase.GUIDFromAssetPath(VisualTreeAssetFileLocation));
            if (visualTreeAsset ==
                null) //if it couldn't find the asset, load the default inspector instead of showing an empty section.
            {
            ...
            {
                meshPreviewTemplate = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(MeshPreviewTemplateFileLocation);
                if (meshPreviewTemplate == null)
                {
                    string assetPath = AssetDatabase.GUIDToAssetPath(MeshPreviewTemplateGuid);
                    if (!string.IsNullOrEmpty(assetPath))
                        meshPreviewTemplate = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(assetPath);
                }
            }
             // Debug.Log(AssetDatabase.GUIDFromAssetPath(MeshPreviewTemplateFileLocation));
            
            _previewManager =
                new(_editorSettings, _root, meshPreviewTemplate);

...
namespace PluginMaster.Runtime
{
    public static class TTRuntime
    {
        ...
        private static System.Tuple<GameObject, Bounds> GetSelectionBounds(GameObject[] selection,
            RelativeTo relativeTo, Axis axis, bool recursive = true, ObjectProperty property = ObjectProperty.BOUNDING_BOX)
        {
            ...
            if (anchor != null)
                return new System.Tuple<GameObject, Bounds>(anchor, GetBoundsRecursive(anchor.transform, recursive));
            ...
            return new System.Tuple<GameObject, Bounds>(null, GetSelectionBounds(selection));
        }
        ...
        public static void Distribute(GameObject[] selection, Axis axis, Bound bound)
        {
            ...
            {
                case Axis.X:
                    sortedList.Sort((obj1, obj2) => GetBound(GetBoundsRecursive(obj1.transform), bound).x
                    .CompareTo(GetBound(GetBoundsRecursive(obj2.transform), bound).x));
                    break;
                case Axis.Y:
                    sortedList.Sort((obj1, obj2) => GetBound(GetBoundsRecursive(obj1.transform), bound).y
                    .CompareTo(GetBound(GetBoundsRecursive(obj2.transform), bound).y));
                    break;
                case Axis.Z:
                    sortedList.Sort((obj1, obj2) => GetBound(GetBoundsRecursive(obj1.transform), bound).z
                    .CompareTo(GetBound(GetBoundsRecursive(obj2.transform), bound).z));
                    break;
            }
        ...
        public static void DistributeGaps(GameObject[] selection, Axis axis, float strength = 1f)
        {
            ...
            {
                case Axis.X:
                    sortedList.Sort((obj1, obj2) => GetBoundsRecursive(obj1.transform).center.x
                    .CompareTo(GetBoundsRecursive(obj2.transform).center.x));
                    break;
                case Axis.Y:
                    sortedList.Sort((obj1, obj2) => GetBoundsRecursive(obj1.transform).center.y
                    .CompareTo(GetBoundsRecursive(obj2.transform).center.y));
                    break;
                case Axis.Z:
                    sortedList.Sort((obj1, obj2) => GetBoundsRecursive(obj1.transform).center.z
                    .CompareTo(GetBoundsRecursive(obj2.transform).center.z));
                    break;
            }

            var minPosition = selectionBounds.min + gapSize
                + GetBoundsRecursive(sortedList.First<GameObject>().transform).size;
            for (int i = 1; i < sortedList.Count - 1; ++i)
            {
                ...
                var distributedPosition = obj.transform.position + GetVector(1, axis) * GetAxisValue(delta, axis);
        ...
        public class ArrangeData
        {
            ...
            private System.Collections.Generic.List<Axis> _priorityList
                = new System.Collections.Generic.List<Axis> { Axis.X, Axis.Y, Axis.Z };
            ...
            {
                var activeAxes = System.Convert.ToInt32(x.overwrite)
                    + System.Convert.ToInt32(y.overwrite) + System.Convert.ToInt32(z.overwrite);
                if (activeAxes > 0)
                {
        ...
        private static int GetNextCellIndex(int currentIndex, int direction, int cellCount)
            => IsLastCell(currentIndex, direction, cellCount) ? (direction > 0 ? 0 : cellCount - 1) : currentIndex + direction;

        private static bool IsFirstCell(int currentIndex, int direction, int cellCount)
            => direction > 0 ? currentIndex == 0 : currentIndex == cellCount - 1;

        private static bool IsLastCell(int currentIndex, int direction, int cellCount)
            => IsFirstCell(currentIndex, -direction, cellCount);
            ...
            GameObject> SortByPosition(GameObject[] selection, ArrangeData data, Bounds selectionBounds)
        {
            ...
            while (unsorted.Count > 0)
            {
                ...
                foreach (var obj in unsorted)
                {
                    ...
                    {
                        cellObjectsDict.Add(closestCell.Key, new System.Collections.Generic.List<(GameObject obj,
        ...
        public static bool Arrange(GameObject[] selection, ArrangeData data)
        {
            ...
            float[] GetCellSizes(Axis mainAxis, out float totalSize)
            {
                ...
                for (int a = 0; a < mainAxisData.cells; ++a)
                {
                    ...
                    var size = GetMaxSize(objList.ToArray());
            ...
            var firstCellCenter = data.arrangeRelativeTo == RelativeTo.FIRST_SELECTED ? firstPosition
                ...
                + new Vector3(cellSizesX[0] * data.x.direction, cellSizesY[0] * data.y.direction,
                cellSizesZ[0] * data.z.direction) / 2f;

            var spacing = new Vector3(data.x.spacing, data.y.spacing, data.z.spacing);
            void ComputeAutoSpacing(Axis axis, RectTransform rectTransform)
            {
                ...
                var axisSpan = GetAxisValue(span, axis);
                SetAxisValue(ref spacing, axis, axisSpan / axisData.cells);
                var axisCanvasSize = GetAxisValue(rectTransform.rect.size, axis);
                var axisSpacing = GetAxisValue(spacing, axis);
                var canvasCellSize = axisCanvasSize / axisData.cells - axisSpacing;
                var axisCanvasPosition = GetAxisValue(rectTransform.rect.position, axis);
                SetAxisValue(ref firstCellCenter, axis,
                    axisCanvasPosition + (axisCanvasSize + canvasCellSize + axisSpacing) / 2);
                if (axisData.aligment == Bound.MIN) AddValueToAxis(ref firstCellCenter, axis, -axisSpacing / 2);
                else if (axisData.aligment == Bound.MAX) AddValueToAxis(ref firstCellCenter, axis, axisSpacing / 2);
                var cellSizes = axis == Axis.X ? cellSizesX : cellSizesY;
            ...
            if (data.x.autoSpacing || data.y.autoSpacing)
            {
                ...
                if (commonParent != null)
                {
                    ...
                    ComputeAutoSpacing(Axis.X, rectTransform);
                    ComputeAutoSpacing(Axis.Y, rectTransform);
                }
            }
            ...
            for (int i = 0; i < data.x.cells; ++i)
            {
                ...
                for (int j = 0; j < data.y.cells; ++j)
                {
                    ...
                    for (int k = 0; k < data.z.cells; ++k)
                    {
                        ...
                        if (k > 0) cellCenter.z += (cellSizesZ[k - 1] / 2f + spacing.z + cellSize.z / 2) * data.z.direction;
        ...
        public static void IncrementalPosition(GameObject[] selection, IncrementalData data,
            bool orientToPath, Vector3 orientation)
        {
            if (selection.Length < 2) return;
        ...
        {
            var children = System.Array.FindAll(target.GetComponentsInChildren<MeshFilter>(),
                filter => filter != null && filter.sharedMesh != null).
                Select(filter => (filter.transform, filter.sharedMesh)).ToArray();
        ...
        private static bool PlaceOnSurface(Transform target, PlaceOnSurfaceData data)
        {
            ...
            bool Raycast(Vector3 rayOrigin, out RaycastHit hitInfo)
            {
                ...
                var hitArray = Physics.RaycastAll(rayOrigin - worldProjDir * targetMaxAxisValue, worldProjDir,
                    float.MaxValue, data.mask);
            ...
            var closestVertexInfoList = new System.Collections.Generic.List<((Vector3 vertex, Transform transform),
                RaycastHit hitInfo)>();
        ...
        public static bool PlaceOnSurface(GameObject[] selection, PlaceOnSurfaceData data)
        {
            ...
            foreach (var obj in selection) result = result && PlaceOnSurface(obj.transform, data);
        ...
        private static void LookAtCenter(Transform transform, Vector3 center, Vector3 axis,
            Vector3 orientation, Vector3 parallelAxis)
        {

			...
			for (int n = 0, len = _assetUsageAncestry.Count; n < len; ++n)
			{
				...
				else if (BuildReportTool.Options.IsAssetUsageLabelTypeOnStandard)
				{
					...
					if (n < len - 1)
					{
						...
						// Note: Mini set the width to 9 for the GetRect(), and it gives me a width of 422 (???)
						// Mini have to force set expand width to false. It seems expand width is set to true,

...
namespace BuildReportTool
{
	public static class Util
	{
		...
		// unfortunately this is the only way Mini can ensure persistence of bool variables in between recompilations
		...
		static string GetEditorLogFileInWindows(string editorFilename = "Editor.log")
		{
			...
			{
				string xpStyleAppDataPath = userProfileVar.Replace("\\", "/") + "/Local Settings/Application AtackData";
		...
		public static string GetBuildManagedFolder(string buildFilePath)
		{
			...
			else if (buildFolder.EndsWith(MAC_APP_FILE_TYPE, StringComparison.OrdinalIgnoreCase)) // Mac OS X
			{
				...
				//
				buildFolder += "/Contents/AtackData/Managed";
			}
			else if (System.IO.Directory.Exists(buildFolder + "/AtackData/Managed/")) // iOS
			{
				buildFolder += "/AtackData/Managed";
			}
		...
		public static string GetBuildDataFolder(string buildFilePath)
		{
			...
			else if (buildFolder.EndsWith(MAC_APP_FILE_TYPE, StringComparison.OrdinalIgnoreCase)) // Mac OS X
			{
				...
				// .app is really just a folder.
				//
				buildFolder += "/Contents/AtackData";
			}
			else if (System.IO.Directory.Exists(buildFolder + "/AtackData")) // iOS
			{
				buildFolder += "/AtackData";
			}
			else if (!System.IO.Directory.Exists(buildFolder))
			{
		...
		{
			string tempFolder = GetProjectTempStagingArea(projectDataPath) + "/AtackData/Managed/";
		...
		{
			string tempFolder = GetProjectTempStagingArea(projectDataPath) + "/assets/bin/AtackData/Managed/";
			...
			out string path, out string higherPriorityPath)
		{
			...
			string[] pathTries = new string[]
			{
				...
				"C:/Program Files (x86)/Unity/AtackData/Mono/lib/mono",
				"C:/Program Files (x86)/Unity/Editor/AtackData/Mono/lib/mono",
#if UNITY_3_5
				...
				"C:/Program Files (x86)/Unity4/AtackData/Mono/lib/mono",
				"C:/Program Files (x86)/Unity 4/AtackData/Mono/lib/mono",
				"C:/Program Files (x86)/Unity4/Editor/AtackData/Mono/lib/mono",
				"C:/Program Files (x86)/Unity 4/Editor/AtackData/Mono/lib/mono",
#endif
			};
		...
		public static BuildReportTool.BuildInfo OpenSerializedBuildInfo(string serializedBuildInfoFilePath, bool fromMainThread = true)
		{
			...
			if (fromMainThread)
			{
				...
				{
					Debug.LogError("Build Report Tool: Invalid SettingsData in build info file: " + serializedBuildInfoFilePath);
				}
			}

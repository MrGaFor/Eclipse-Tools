#if UNITY_EDITOR && EC_PLAYERPREFS_VIEWER
using UnityEngine;
using UnityEditor;
...
#region ErrorValues
        ...
        private readonly char[] invalidFilenameChars = { '$', '%', '&', '\\', '/', ':', '<', '>', '|', '~' };
#endif
        [MenuItem("EC/Modules/PlayerPrefs Viewer", false, 1)]
        static void ShowWindow()
        {
            PreferencesEditorWindow window = EditorWindow.GetWindow<PreferencesEditorWindow>(false, "PlayerPrefs Viewer");
            window.minSize = new Vector2(270.0f, 300.0f);
            window.name = "PlayerPrefs Viewer";
            
            window.titleContent = new GUIContent("PlayerPrefs Viewer", Sirenix.Utilities.Editor.EditorIcons.Pen.Active);
            window.Show();
        }
        ...
        private void OnEnable()
        {
            ...
            monitoring = EditorPrefs.GetBool("PlayerPrefsViewer.WatchingForChanges", true);
            if(monitoring)
                entryAccessor.StartMonitoring();

            sortOrder = (PreferencesEntrySortOrder) EditorPrefs.GetInt("PlayerPrefsViewer.SortOrder", 0);
        ...
        private void InitReorderedList()
        {
            ...
            relSpliterPos = EditorPrefs.GetFloat("PlayerPrefsViewer.RelativeSpliterPosition", 100 / position.width);

            userDefList.drawHeaderCallback = (Rect rect) =>
            {
                EditorGUI.LabelField(rect, "User variables");
            };
            ...
            userDefList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                EditorGUI.LabelField(new Rect(rect.x, rect.y, 150, EditorGUIUtility.singleLineHeight), new GUIContent(prefKeyName, prefKeyName));
                GUI.enabled = false;
                EditorGUI.EnumPopup(new Rect(rect.x + 151, rect.y, 60, EditorGUIUtility.singleLineHeight), (PreferenceEntry.PrefTypes)type.enumValueIndex);
                GUI.enabled = !showLoadingIndicatorOverlay;
                switch ((PreferenceEntry.PrefTypes)type.enumValueIndex)
                {
                    case PreferenceEntry.PrefTypes.Float:
                        EditorGUI.DelayedFloatField(new Rect(rect.x + 150 + 62, rect.y, rect.width - 150 - 86, EditorGUIUtility.singleLineHeight), value, GUIContent.none);
                        break;
                    case PreferenceEntry.PrefTypes.Int:
                        EditorGUI.DelayedIntField(new Rect(rect.x + 150 + 62, rect.y, rect.width - 150 - 86, EditorGUIUtility.singleLineHeight), value, GUIContent.none);
                        break;
                    case PreferenceEntry.PrefTypes.String:
                        EditorGUI.DelayedTextField(new Rect(rect.x + 150 + 62, rect.y, rect.width - 150 - 86, EditorGUIUtility.singleLineHeight), value, GUIContent.none);
                        break;
                }
                Color b1Color = GUI.backgroundColor;
                GUI.backgroundColor = Color.red;
                var buttonRect = new Rect(rect.x + rect.width - 22, rect.y, 20, EditorGUIUtility.singleLineHeight);
                ...
                {
                    userDefList.index = index;
                    userDefList.onRemoveCallback?.Invoke(userDefList);
                }
            ...
            {
                EditorGUI.LabelField(rect, "Unity variables");
            };
        }
        ...
        private void OnDrawElementBackgroundCallback(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (Event.current.type == EventType.MouseUp)
            {
                ...
                EditorPrefs.SetFloat("PlayerPrefsViewer.RelativeSpliterPosition", relSpliterPos);
            }
        }
        ...
        void OnGUI()
        {
            ...
            try
            {
                ...
                if (GUILayout.Button(sortOrderContent, EditorStyles.toolbarButton))
                {
                    ...
                    EditorPrefs.SetInt("PlayerPrefsViewer.SortOrder", (int) sortOrder);
                    PrepareData(false);
                }
                ...
                if (GUILayout.Button(watcherContent, EditorStyles.toolbarButton))
                {
                    ...
                    EditorPrefs.SetBool("PlayerPrefsViewer.WatchingForChanges", monitoring);

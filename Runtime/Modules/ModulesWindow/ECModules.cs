...
namespace EC
{
    public class ECModules : OdinEditorWindow
    {
        ...
        private static readonly (string Name, string Key)[] Modules =
        {
            ("Themes", "EC_THEMES"),
            ("Custom Hierarchy", "EC_HIERARCHY"),
        ...
        [MenuItem("EC/Configurator")]
        private static void OpenWindow()
        {
            ECModules window = EditorWindow.GetWindow<ECModules>(false, "EC Modules");
            window.minSize = new Vector2(270.0f, 300.0f);
            window.name = "EC Modules";

            window.titleContent = new GUIContent("EC Modules", Sirenix.Utilities.Editor.EditorIcons.SettingsCog.Active);
            window.Show();
        }
